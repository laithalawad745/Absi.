...
قومبز جيم 
          ...
          {/* <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl p-4 mb-6 shadow-2xl border border-slate-700">
            <h3 className="text-lg font-bold text-center text-slate-200 mb-3">إحصائيات الأسئلة</h3>
            <div className="flex justify-center items-center gap-4 text-sm md:text-base">
              <span className="text-blue-400">المستخدمة: {stats.used}</span>
              <span className="text-slate-400">من أصل: {stats.total}</span>
              <span className="text-green-400">النسبة: {stats.percentage}%</span>
            </div>
            {stats.used > 0 && (
              <div className="mt-3 text-center">
                <button
                  onClick={() => resetGame(true)}
                  className="bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white px-4 py-2 rounded-lg font-bold text-sm transition-all duration-300"
                >
                  مسح جميع الأسئلة المستخدمة
                </button>
              </div>
            )}
          </div> */}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-2 md:p-4 select-none">
      ...
      <div className={`max-w-7xl mx-auto ${currentQuestion ? 'pt-16 md:pt-20' : ''}`}>
        ...
        <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl p-3 md:p-8 shadow-2xl border border-slate-700">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 md:gap-6">
            {selectedTopics.map(topic => (
                    ...
                    {['easy', 'medium', 'hard'].map(difficulty => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* {!hasTeamUsedThisLevel && availableCount > 0 && availableCount < 3 && (
                            <span className="block text-xs opacity-75">({availableCount})</span>
                          )} */}
                        </button>
                      );
                    })}
